...
function workflowApiServiceFactory(
    ...
    var findGraphDefinitionByNameAVG =0
    var findGraphDefinitionByNameARR = []
    WorkflowApiService.prototype.findGraphDefinitionByName = function (graphName) {
        var start = new Date().getTime()
        console.time("findGraphDefinitionByName")
        //return taskGraphStore.getGraphDefinitions(graphName) //<<<<<<<<<<<<<<<<<<<<<<<<<<
        return taskGraphStore.getGraphDefinitions1(graphName) //<<<<<<<<<<<<<<<<<<<<<<<<<<
            .then(function (graph) {
                if (_.isEmpty(graph)) {
            ...
            .tap( console.timeEnd.bind(console,'findGraphDefinitionByName'))
            .tap (function(){

                var end = new Date().getTime()
                var e = end-start
                var sum = 0
                findGraphDefinitionByNameARR.push(e)

                if(findGraphDefinitionByNameARR.length == 1000) {
                    findGraphDefinitionByNameARR.forEach(function (item) {
                        sum = sum + item
                    })
                    //console.log("sum on crearteAndRun", sum)
                    findGraphDefinitionByNameAVG = sum / findGraphDefinitionByNameARR.length
                    console.log("time_array of findGraphDefinitionByNameARR: " + findGraphDefinitionByNameARR)
                    console.log("time_avg of findGraphDefinitionByNameAVG: " + findGraphDefinitionByNameAVG)
                }
    ...
    var graph_persistAVG =0
    var graph_persistArr = []
    ...
    WorkflowApiService.prototype.createActiveGraph = function (
        ...
        .then(function (graph) {
            ...
            //return graph.persist2()

            //#No waterline but without queueing
            return graph.persist1()
        ...
        .tap(console.timeEnd.bind(console, 'create taskActiveGraph'))
            .tap( function(){
                ...
                var sum = 0
                console.timeEnd('createAndRunGraph')
                graph_persistArr.push(e)
                if(graph_persistArr.length == 1000) {
                    graph_persistArr.forEach(function (item) {
                        sum = sum + item
                    })
                    //console.log("sum on crearteAndRun", sum)
                    graph_persistAVG = sum / graph_persistArr.length
                    console.log("time_array of persist: " + graph_persistArr)
                    console.log("time_avg of perstist: " + graph_persistAVG)
                }
            })
    };
